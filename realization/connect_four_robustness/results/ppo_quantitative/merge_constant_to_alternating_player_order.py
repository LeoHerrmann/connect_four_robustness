...
learning_rate = "0_0001"

path_to_absolute_history_ppo_vs_random = "ppo_vs_random_constant_player_order/ppo_vs_random_" + learning_rate + "/absolute_history.json"
path_to_absolute_history_random_vs_ppo = "random_vs_ppo_constant_player_order/random_vs_ppo_" + learning_rate + "/absolute_history.json"
destination_path_for_average_history = "ppo_vs_random_alternating_player_order/" + learning_rate + "/average_history.json"
destination_path_for_absolute_history = "ppo_vs_random_alternating_player_order/" + learning_rate + "/absolute_history.json"
destination_path_for_average_win_rate_graph = "ppo_vs_random_alternating_player_order/" + learning_rate + "/win_rate.png"
destination_path_for_average_game_length_graph = "ppo_vs_random_alternating_player_order/" + learning_rate + "/game_length.png"
...
def generate_average_history_figures(average_history: list[dict]):
    ...
    player_0_win_rates = [(item["win_rate_ppo"] * 100) for item in average_history]
    ...
    plt.plot(game_indices, player_0_win_rates, label="PPO")
...
average_history_ppo_vs_random = []
average_history_random_vs_ppo = []
absolute_history_ppo_vs_random = []
absolute_history_random_vs_ppo = []

with open(path_to_absolute_history_ppo_vs_random) as f:
    absolute_history_ppo_vs_random = json.load(f)["absolute_history"]

with open(path_to_absolute_history_random_vs_ppo) as f:
    absolute_history_random_vs_ppo = json.load(f)["absolute_history"]


# Sicherstellen, dass die geladenen absoluten Historien gleich lang sind
if len(absolute_history_ppo_vs_random) != len(absolute_history_random_vs_ppo):
...
for i in range(len(absolute_history_ppo_vs_random)):
	new_history_item_1 = {
		"winner": "PPO" if absolute_history_ppo_vs_random[i]["winner"] == "player_0" else "Zufällig",
		"game_length": absolute_history_ppo_vs_random[i]["game_length"]
	}
	
	new_history_item_2 = {
		"winner": "PPO" if absolute_history_random_vs_ppo[i]["winner"] == "player_1" else "Zufällig",
		"game_length": absolute_history_random_vs_ppo[i]["game_length"]
	}
...
for i in range(len(merged_absolute_history)):
	...
	if merged_absolute_history[i]["winner"] == "PPO":
		total_wins_ppo += 1
	elif merged_absolute_history[i]["winner"] == "Zufällig":
		total_wins_random += 1
	
	new_history_item = {
		"win_rate_ppo": total_wins_ppo / game_count,
