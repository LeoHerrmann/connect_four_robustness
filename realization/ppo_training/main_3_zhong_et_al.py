...
def train_action_mask(env_fn, model_tuples: list[tuple[ModelWrapper, ModelWrapper]], steps=10_000, seed=0, **env_kwargs):
    ...
    for model_tuple in model_tuples:
        ...
        # model_tuple[0].model.save(f"weights_from_main_zhong_et_al/{model_tuple[0].name}_{time.strftime('%Y%m%d-%H%M%S')}")
        ...
        # model_tuple[1].model.save(f"weights_from_main_zhong_et_al/{model_tuple[1].name}_{time.strftime('%Y%m%d-%H%M%S')}")
...
def save_and_load_model_tuples(model_tuples: list[tuple[ModelWrapper, ModelWrapper]]) -> list[tuple[ModelWrapper, ModelWrapper]]:
    ...
    for model_tuple in model_tuples:
        model_tuple[0].model.save(f"weights_from_main_zhong_et_al/{model_tuple[0].name}_{time_string}")
        copied_model_0 = MaskablePPO.load(f"weights_from_main_zhong_et_al/{model_tuple[0].name}_{time_string}")
        copied_model_wrapper_0 = ModelWrapper(model_tuple[0].name, copied_model_0)

        model_tuple[1].model.save(f"weights_from_main_zhong_et_al/{model_tuple[1].name}_{time_string}")
        copied_model_1 = MaskablePPO.load(f"weights_from_main_zhong_et_al/{model_tuple[1].name}_{time_string}")
