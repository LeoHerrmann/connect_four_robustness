...
class SB3ActionMaskWrapper(pettingzoo.utils.BaseWrapper):
    ...
    def __init__(self, env, stationary_model: MaskablePPO):
        super().__init__(env)
    ...
    def reset(self, seed=None, options=None):
        ...
        if not self.training_agent_is_player_0:
            ...
            if self.stationary_model is None:
                # If no model is given execute step based on random policy
                ...
                # If a model is given execute step based on given model
...
def train_action_mask(env_fn, steps=10_000, seed=0, stationary_model: MaskablePPO = None, training_model: MaskablePPO = None, **env_kwargs):
    ...
    env = SB3ActionMaskWrapper(env, stationary_model)
    ...
    training_model.save(f"weights_from_main_with_frozen_agents/{env.unwrapped.metadata.get('name')}_{time.strftime('%Y%m%d-%H%M%S')}")
...
def try_get_latest_policy_path():
    try:
        ...
        latest_policy_path = max(
            glob.glob(f"weights_from_main_with_frozen_agents/{env.metadata['name']}*.zip"), key=os.path.getctime
        )
...
def execute_self_play_training_loop(
    ...
    for i in range(number_of_iterations):
        ...
        # Evaluate against stationary model
        ...
        # Update weights of stationary model
        if winrate_stationary > threshold_winrate_for_updating_stationary_model:
            ...
            stationary_model = MaskablePPO.load(latest_policy_path)
...
def save_training_progress_data(training_progress_data: list[dict]):
    history_file_name = f"histories/history_frozen_{time.strftime('%Y%m%d-%H%M%S')}.json"
